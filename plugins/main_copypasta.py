...
for token in tokenlist:
    ...
    if sys.platform.startswith('win32'):# for windows
        p = subprocess.Popen(['python','.\\plugins\\copypastaspam.py',token,number,text,chan],stdout=open(os.devnull, "w"), stderr=subprocess.STDOUT) #Make sure you add the token and server after 'python' and the path, which should be in the plugins folder.
    elif sys.platform.startswith('linux'):# for linux
        p = subprocess.Popen(['python','plugins/copypastaspam.py',token,number,text,chan],stdout=open(os.devnull, "w"), stderr=subprocess.STDOUT)
